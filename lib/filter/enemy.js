  ...
  for (var round in json) {
    ...
    for (var enemy in json[round].enemy) {
      for (var part in json[round].enemy[enemy].children) {
        ...
        json[round].enemy[enemy].children[part].params[0].def_ttributes = [];
        rows.push('Debuffed');
    ...
    var drops = 0;
    for (enemy in json[round].enemy) {
      if (json[round].enemy[enemy].children[part].drop_item_list.length > 0) {
        drops = 1;
        for (var item in json[round].enemy[enemy].children[part].drop_item_list) {
          if (json[round].enemy[enemy].children[part].drop_item_list[item].type == 11) {
            console.log(
              '\Gold: ' + json[round].enemy[enemy].children[part].drop_item_list[item].amount
            );
          } else {
            console.log(
              '\Name: ' + inventory[json[round].enemy[enemy].children[part].drop_item_list[item].item_id],
              '- Round: ' + json[round].round, 
              '- Rarity: ' + json[round].enemy[enemy].children[part].drop_item_list[item].rarity
            );
          }
        }
      } 
