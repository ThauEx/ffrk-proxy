 ...
 * Modified on 2/4/2016 by SirPhoenix88
 */
...
exports.update = function (json) {
  ...
  for (var buddy in json) {
    ...
    json[buddy].params[0].atk = config.get('buddy.stats.atk');
    tmpRow.push(json[buddy].params[0].atk);

    json[buddy].params[0].def = config.get('buddy.stats.def');
    tmpRow.push(json[buddy].params[0].def);

    json[buddy].params[0].matk = config.get('buddy.stats.matk');
    tmpRow.push(json[buddy].params[0].matk);

    json[buddy].params[0].mdef = config.get('buddy.stats.mdef');
    tmpRow.push(json[buddy].params[0].mdef);

    json[buddy].params[0].mnd = config.get('buddy.stats.mnd');
    tmpRow.push(json[buddy].params[0].mnd);

    json[buddy].params[0].acc = config.get('buddy.stats.acc');
    tmpRow.push(json[buddy].params[0].acc);

    json[buddy].params[0].eva = config.get('buddy.stats.eva');
    tmpRow.push(json[buddy].params[0].eva);

    json[buddy].params[0].spd = config.get('buddy.stats.spd');
    tmpRow.push(json[buddy].params[0].spd);

    /**
     * All attacks ranged
     */
    json[buddy].params[0].atk_type = atkType[config.get('buddy.attack.type')];
    tmpRow.push(config.get('buddy.attack.type'));
