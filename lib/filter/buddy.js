...
exports.update = function (json) {
  ...
  for (var buddy in json) {
    ...
    tmp = json[buddy].params[0].mnd;
    json[buddy].params[0].mnd = 9999;
    tmpRow.push(/*tmp + '/'*/ + json[buddy].params[0].mnd);
    ...
    json[buddy].params[0].atk_attributes = [
      ...
      {attribute_id: '242', factor: 100},     // Stun
    ];
     ...
     * @see comment below
     */
    json[buddy].status_ailments = [204, 207, 208, 216, 240, 508, 509];
    ...
    /*
     * Ability spammer
     * Refills abilities per round and increases max to 10 (or decreases if 12, but it will refill every round)
     * Fills soulbreak gauge one full unit per ability
     */
    for (var panel in json[buddy].ability_panels) {
      json[buddy].ability_panels[panel].num = '10';
      json[buddy].ability_panels[panel].max_num = '10';
