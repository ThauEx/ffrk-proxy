...
exports.update = function (json) {
    var atk_type = {'1':'normal','2':'ranged'},

    table = new Table({
        head: ['Name','id','no','HP','atk','def','matk','mdef','acc','eva','spd','atk_type','effect'],
    ...
    for (var buddy in json) {
        ...
        **/
        ...
        json[buddy].params[0].atk_type = '2';
        tmpRow.push(atk_type[json[buddy].params[0].atk_type+'']);
        ...
        **/
        ...
        json[buddy].abilities[0].options.target_method = 6; //Hits everyone
        atk_names.push(tmpRow[0]);
        for(var ability in json[buddy].abilities){
            json[buddy].abilities[ability].options.cast_time = '100'; //Quick attack
            atk_names.push(json[buddy].abilities[ability].options.name+'='+
                            json[buddy].abilities[ability].options.cast_time);
        }
        atk_names.push("\n");
        tmpRow.push('Bad Breath + Quick Hit');
