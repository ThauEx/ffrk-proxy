      ...
      if (domains.hasOwnProperty(clientReq.headers.host)) {
        if (clientReq.url.indexOf('/battle/?') > -1) {
          transform = function(content, callback) {
            ...
            // content = content.replace('(function(global){', '(function(global){\n' +
            //   'global.FF.env.isDevelop = function() {return true;}');
            content = content.replace('</body>', '<script type="application/javascript" src="' + jsPath + '"></script>' +
