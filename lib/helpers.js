...
function makeSNICallback() {
    var caKey = fs.readFileSync(__dirname + '/../cert/root/rootCA.key', 'utf8'),
        caCert = fs.readFileSync(__dirname + '/../cert/root/rootCA.crt', 'utf8'),
    ...
    return function SNICallback(servername, cb) {
        if(!cache.hasOwnProperty(servername)) {
            ...
            var certOptions = servername;
            cache[servername] = createCertificateAsync(certOptions, caCert, caKey, serial++).then(function (pem) {
