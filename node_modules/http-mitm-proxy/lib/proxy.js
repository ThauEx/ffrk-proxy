...
Proxy.prototype.listen = function(options) {
  ...
  this.sslCertCacheDir = options.sslCertCacheDir || path.resolve(homedir, '.http-mitm-proxy');
  this.sslServers = {};
...
Proxy.prototype.use = function(mod) {
  ...
  if (mod.onResponseEnd) {
    this.onResponseEnd(mod.onResponseEnd);
  }
};
  ...
  this.end = function(chunk) {
    if (chunk) {
      return proxy._onResponseData(ctx, chunk, function(err, chunk) {
        ...
        return proxy._onResponseEnd(ctx, chunk, function (err, chunk) {
          if (err) {
            return proxy._onError(ctx, err);
          }   
...
Proxy.prototype._onResponseEnd = function(ctx, chunk, callback) {
  var self = this;
